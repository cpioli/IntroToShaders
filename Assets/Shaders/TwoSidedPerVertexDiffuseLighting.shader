Shader ".ShaderTalk/TwoSidedPerVertexLighting" {
	Properties {
	...
	SubShader {
		Pass {
			...
			vertexOutput vert(vertexInput input)
			{
				...
				if (0.0 == _WorldSpaceLightPos0.w) //if it's a direction light
				{
					attenuation = 1.0;
					lightDirection = normalize(_WorldSpaceLightPos0.xyz);
				}
				else //it's a point light or a spot light
				{
				...
				if (dot(normalDirection, lightDirection) < 0.0) //light source on incorrect side
				{
		...
		Pass {
      	...
      	Pass {
      		...
      		vertexOutput vert(vertexInput input)
      		{
      			...
      			else //a point light or a spot light
      			{
      				float3 vertexToLightSource = _WorldSpaceLightPos0.xyz
      					- mul(modelMatrix, input.vertex).xyz;
      				float distance = length(vertexToLightSource);
      			...
      			float3 specularReflection;
      			if (dot(normalDirection, lightDirection) < 0.0) // if light source on the wrong side
      			{
      				//then no specular reflection exists
      				specularReflection = float3(0.0, 0.0, 0.0);
      			}
      			else //if light source is on the correct side
      			{
      				specularReflection = attenuation * _LightColor0.rgb
      	...
      	Pass {
      		...
      		vertexOutput vert(vertexInput input)
      		{
      			...
      			float4x4 matrixModel = _Object2World;
      			float4x4 matrixModelInverse = _World2Object;

      			float3 normalDirection = normalize(
      				mul(float4(-input.normal, 0.0), matrixModelInverse).xyz);
      			float3 viewDirection = normalize(_WorldSpaceCameraPos
      				- mul(matrixModel, input.vertex).xyz);
      			float3 lightDirection;
      			float attenuation;

      			if (0.0 == _WorldSpaceLightPos0.w) // if it is a directional light
      			{
      				attenuation = 1.0;
      				lightDirection = normalize(_WorldSpaceLightPos0.xyz);
      			}
      			else
      			{
      				float3 vertexToLightSource = _WorldSpaceLightPos0.xyz
      					- mul(matrixModel, input.vertex).xyz;
      				float distance = length(vertexToLightSource);
      				attenuation = 1.0 / distance; //linear attentuation
      			}
      			...
      			float3 specularReflection;
      			if (dot(normalDirection, lightDirection) < 0.0) 
      				//if the dot product returned 0.0
      				// meaning the light direction is on the wrong side
      			{
      				specularReflection = float3(0.0, 0.0, 0.0); //no specular reflection
      			}
      			else // if the light source is on the correct side
      			{
      				specularReflection = attenuation * _LightColor0.rgb
      					* _BackSpecColor.rgb * pow(max(0.0, dot(
      					reflect(-lightDirection, normalDirection),
