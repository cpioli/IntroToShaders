Shader ".ShaderTalk/PerVertexLightingTexture" {
	Properties {
		...
		Pass {
			Tags { "LightMode" = "ForwardBase" }
				...
				vertexOutput vert(vertexInput input)
				{
					...
					else //if it's a point or spot light
					{
			...
			Pass {
				...
				#include "UnityCG.cginc"
				uniform float4 _LightColor0;
					// color of light source (from "Lightning.cginc")
				...
				vertexOutput vert(vertexInput input)
				{
					...
					{
						attenuation = 1.0; //if so, then no attenuation is required
						lightDirection = normalize(_WorldSpaceLightPos0.xyz);
					}
					else // otherwise if it is a point or spot light
					{
						float3 vertexToLightSource = _WorldSpaceLightPos0.xyz
							- mul(modelMatrix, input.vertex).xyz;
						float distance = length(vertexToLightSource);
					...
					float3 specularReflection;
					if (dot(normalDirection, lightDirection) < 0.0) // if the light source is on the wrong side
					{
						specularReflection = float3(0.0, 0.0, 0.0);
						// then we don't need any specular reflection
					}
					else
					//if the light source is on the correct side
					{
						specularReflection = attenuation * _LightColor0.rgb * _SpecColor.rgb *
							pow(max(0.0, dot(reflect(-lightDirection, normalDirection), viewDirection)), _Shininess);
					}
